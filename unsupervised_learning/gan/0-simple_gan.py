...
class Simple_GAN(keras.Model) :
    ...
    def __init__( self, generator , discriminator , latent_generator, real_examples, batch_size=200, disc_iter=2, learning_rate=.005):
        ...
        (x, tf.ones(x.shape))+ tf.keras.losses.MeanSquaredError()(y, -1*tf.ones
                (y.shape))
    ...
    def train_step(self,useless_argument):
        for _ in range(self.disc_iter) :
            ...
            self.discriminator.optimizer.apply_gradients(zip(g.gradient(discr_loss,self.discriminator.weights),self.discriminator.weights))
